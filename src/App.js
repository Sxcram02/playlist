  ...
  return (
    <div className="component">
      ...
      <div className="titleContainer">
        ...
        <div className="time">
          {currentTime.sec < 10 ? (<p>{currentTime.min}:0{currentTime.sec}</p>) : (<p>{currentTime.min}:{currentTime.sec}</p>)}
          <p>{time.min}:{time.sec}</p>
        </div>
        <input type="range" min={0} max={duration/60} defaultValue={0} value={seconds} onChange={(e)=> { sound.seek(e.target.value)}} /> 
      </div>
      <div>
